...
async def update_objects_tags(request, objects_tags_data, check_ids = False):
    ...
    if "object_ids" in objects_tags_data:
        ...
        if len(tag_updates["removed_tag_ids"]) > 0: # Log here instead of _remove_tags_for_objects to avoid logging updates as deletions
            request[request_log_event_key]("INFO", "db_operation", "Removed objects' tags.", details=f"object_ids = {objects_tags_data['object_ids']}, tag_ids = {tag_updates['removed_tag_ids']}")
    ...
    # Update objects for tags
        ...
        if len(object_updates["removed_object_ids"]) > 0: # Log here instead of _remove_tags_for_objects to avoid logging updates as deletions
            request[request_log_event_key]("INFO", "db_operation", "Removed tags' objects.", details=f"tag_ids = {objects_tags_data['tag_ids']}, object_ids = {object_updates['removed_object_ids']}")
...
async def _add_tags_for_objects(request, objects_tags_data):
    ...
    if len(tag_ids) > 0:
        ...
        if len(non_existing_tag_ids) > 0:
            ...
            request[request_log_event_key]("WARNING", "db_operation", msg, details=f"tag_ids = {non_existing_tag_ids}")
    ...
    if request[request_user_info_key].user_level != "admin":
        ...
        if (await result.fetchone())[0] > 0:
            request[request_log_event_key]("WARNING", "db_operation", "Attempted to add non-published tags as a non-admin.")
    ...
    if len(tag_names) > 0:
        ...
        request[request_log_event_key]("INFO", "db_operation", "Added new tags during objects' tag update.", details=f"tag_ids = {new_tag_ids}")
    ...
    request[request_log_event_key]("INFO", "db_operation", "Updated objects' tags.", details=f"object_ids = {objects_tags_data['object_ids']} tag_ids = {tag_ids}")
    return list(tag_ids)
...
async def _remove_tags_for_objects(request, objects_tags_data):
    ...
    if request[request_user_info_key].user_level != "admin":
        ...
        if (await result.fetchone())[0] > 0:
            request[request_log_event_key]("WARNING", "db_operation", "Attempted to delete non-published tags as a non-admin.")
...
async def _add_objects_for_tags(request, objects_tags_data):
    ...
    request[request_log_event_key]("INFO", "db_operation", "Updated tags' objects.", details=f"tag_ids = {objects_tags_data['tag_ids']} object_ids = {added_object_ids}")
...
async def _check_object_ids(request, checked_object_ids):
    ...
    if len(non_existing_object_ids) > 0:
        ...
        request[request_log_event_key]("WARNING", "db_operation", msg, details=f"object_ids = {non_existing_object_ids}")
        raise RequestValidationException(msg)
