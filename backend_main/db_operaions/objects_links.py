...
async def add_link(request, obj_id_and_data):
    new_link = {"object_id": obj_id_and_data["object_id"], "link": obj_id_and_data["object_data"]["link"]}
...
async def update_link(request, obj_id_and_data):
    new_link = {"object_id": obj_id_and_data["object_id"], "link": obj_id_and_data["object_data"]["link"]}
    validate_link(new_link["link"])
    ...
    await request["conn"].execute(
        links.update()
        .where(links.c.object_id == obj_id_and_data["object_id"])
        .values(new_link)
    ) 
