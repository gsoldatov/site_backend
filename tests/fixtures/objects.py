...
def get_test_object(object_id, object_type = None, created_at = None, modified_at = None, \
    ...
    obj = {
        ...
        "feed_timestamp": feed_timestamp if feed_timestamp is not None else curr_time.isoformat() + "Z",    # TODO remove z?
...
def get_test_object_data(object_id, object_type = "link", composite_subobject_object_type = None):
    ...
    If `composite_subobject_object_type` is set, composite object is returned with its first subobject containing object attributes & data of the specified type.
    """
    if object_type == "link": return {"object_id": object_id, "object_data": _get_link_object_data(object_id)}
    if object_type == "markdown": return {"object_id": object_id, "object_data": _get_markdown_object_data(object_id)}
    if object_type == "to_do_list": return {"object_id": object_id, "object_data": _get_to_do_list_object_data(object_id)}
    if object_type == "composite": return {"object_id": object_id, "object_data": _get_composite_object_data(object_id, composite_subobject_object_type)}
